...
export function AnalyticsClient({ data }: { data: AnalyticsData }) {
  ...
  return (
    <div className="space-y-6">
          ...
          return (
            <Card key={k.label} className="hover:shadow-md transition-shadow">
              <CardContent className="pt-5 pb-4">
                ...
                <div className={cn('flex items-center gap-0.5 text-[11px] font-medium mt-1.5', k.trendUp ? 'text-green-600' : 'text-red-500')}>
      ...
      {/* Charts Row 1: Score Trend + Pass/Fail by Factory */}
        ...
        <Card>
          ...
          <CardContent>
            {monthlyTrend.length > 0 ? (
              ...
              <div className="h-[220px] flex items-center justify-center text-sm text-muted-foreground">Not enough data</div>
            )}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            {factoryPassFail.length > 0 ? (
              ...
              <div className="h-[220px] flex items-center justify-center text-sm text-muted-foreground">No factory data</div>
            )}
          </CardContent>
        </Card>
      </div>

      {/* Charts Row 2: Defect Pie + Project Pie + Inspector Table */}
      <div className="grid lg:grid-cols-3 gap-5">
        {/* Defect Severity */}
        <Card className="h-full">
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold">Defect Severity</CardTitle>
          </CardHeader>
          <CardContent>
            {totalDefects > 0 ? (
              <>
                <ResponsiveContainer width="100%" height={160}>
                  <PieChart>
                    <Pie data={defectDistribution} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" paddingAngle={2}>
              ...
              <div className="h-[200px] flex items-center justify-center text-sm text-muted-foreground">No defects recorded</div>
            )}
          </CardContent>
        </Card>
          ...
          <CardContent>
            {projectStatusData.length > 0 ? (
              <>
                <ResponsiveContainer width="100%" height={160}>
                  <PieChart>
                    <Pie data={projectStatusData} cx="50%" cy="50%" innerRadius={40} outerRadius={65} dataKey="value" paddingAngle={2}>
              ...
              <div className="h-[200px] flex items-center justify-center text-sm text-muted-foreground">No projects created</div>
            )}
          </CardContent>
        </Card>
      ...
      {monthlyTrend.length > 1 && (
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-semibold">Monthly Inspection Volume</CardTitle>
          </CardHeader>
          <CardContent>
            <ResponsiveContainer width="100%" height={180}>
              <BarChart data={monthlyTrend} margin={{ top: 4, right: 8, bottom: 0, left: -20 }}>
                <CartesianGrid strokeDasharray="3 3" stroke="var(--border)" />
                <XAxis dataKey="month" tick={{ fontSize: 11 }} />
                <YAxis tick={{ fontSize: 11 }} />
                <Tooltip contentStyle={TOOLTIP_STYLE} />
